...
def get_logged_in_user(request, pk):
    session_pk = request.session.get('user_pk')
    if session_pk is None or int(session_pk) != int(pk):
        return None
    return get_object_or_404(Account, pk=pk)


def basic_list(request, pk):
    user = get_logged_in_user(request, pk)
    if not user:
...
def view_detail(request, user_pk, pk):
    user = get_logged_in_user(request, user_pk)
    if not user:
        return redirect('login')
